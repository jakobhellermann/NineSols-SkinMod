    ...
    for atlas in input_dir.iterdir():
        ...
        for sprite in atlas.iterdir():
            ...
            if not out_sprite.exists():
                ...
                any_changed = False
                for replace, to, deciding_factor in replacements:
                    any_changed = any_changed or replace_color(data, replace, to, fuzz)
                if any_changed:
                    for replace, to, deciding_factor in replacements_weak:
...
def create_skin(name, out, replacements, replacements_weak):
    recolor(name, out, replacements, replacements_weak, fuzz=50)
...
create_skin(
    ...
    [
        ((231, 174, 51), (231, 51, 60), True),  # cape
    ],
    [
        ((66, 67, 49), (66, 53, 49), False),  # cape border
        ((115, 87, 27), (66, 53, 49), False),  # cape shadow
        ((168, 129, 40), (66, 53, 49), False),  # cape shadow 2
        ((82, 203, 132), (106, 125, 185), False),  # green highlight
    ],
)
...
create_skin(
    ...
    [
        ((231, 174, 51), (91, 52, 235), True),  # cape
    ],
    [
        ((66, 67, 49), (66, 53, 49), False),  # cape border
        ((115, 87, 27), (66, 53, 49), False),  # cape shadow
